...
public class EmployeeController {
     ...
     * 通过RestTemplate调用服务，post请求参数列表：url、参数列表、返回值类型 Eureka存在情况下，该请求会报错
     */
    @GetMapping("/list")
    public List<EmployeeModel> list() {
        return restTemplate.getForObject(ServerContant.REST_URL + "/emp/list", List.class);
    }
    ...
    public List<WebEmployeeModel> list2() {
        ...
        String json = restTemplate.getForObject(ServerContant.SERVER_PROVIDER_BEFORE_HTTP + "/emp/list",
                String.class);
        // 将json字符串转集合
